...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------------- HERO ROTATOR (opening line + press quotes) ----------------
     Pure time-based auto-rotate, independent of scroll. Scrolling never scrubs
     the quotes — it just advances the scroll journey while the hero fades out. */
  const hrEl = $('#heroRotate');
  if (hrEl){
    const items = $$('.hr__item', hrEl);
    if (items.length > 1){
      let idx = 0;
      const go = n => {
      ...
      // linger on the opening statement, then rotate slowly through the quotes
      setTimeout(() => { go(idx + 1); setInterval(() => go(idx + 1), 6000); }, 7500);
    }
  }
